...
app.config['MAIL_USERNAME'] = 'tourmanagementproj@gmail.com' #email
...
SQLALCHEMY_DATABASE_URI = "mysql+mysqlconnector://{username}:{password}@{hostname}/{databasename}".format( #database host address
    username="TourManagement", #account username
    password="ITSASECRET", #account password
    hostname="TourManagement.mysql.pythonanywhere-services.com", #host address
    databasename="TourManagement$users", #table name
)
...
class Tour(db.Model): #tour

    __tablename__ = "tour" #tour table

    id = db.Column(db.Integer, primary_key=True) #unique tour ID
...
class Form(db.Model): #forms
    ...
    user = db.Column(db.String(4096)) #owner
    tour = db.Column(db.String(4096)) #owner
    description = db.Column(db.String(4096)) #description
...
def forgotPassword(): #function to reset password
    if request.method=="GET": #user loads page
        ...
        msg = Message('Password reset for Tour Management Account ' + str(username), sender = 'tourmanagementproj@gmail.com', recipients = [email]) #prepare email to be sent to user
        msg.html = '<h1 style="text-align: center;">Hey ' + str(username) + "!</h1></br>" + "<h2 style='text-align: center;'>We have received your request to reset your password.</h2></br>" + '<h3 style="text-align: center;">Reset your password <a href=' + "https://tourmanagement.pythonanywhere.com/reset_password/" + getHashed(username) + ">here</a>!</h3>" + '</br><h3 style="text-align: center;">If this was not you, kindly ignore it.' '</br><h4 style="text-align: center;">If you have any queries you may send an email to us at tourmanagementproj@gmail.com!</h4>' #code for body of email
...
def forgotUsername(): #function to get username(s)
    if request.method=="GET": #user loads page
        ...
        msg = Message('Tour Management Account Usernames associated with ' + str(email), sender = 'tourmanagementproj@gmail.com', recipients = [email]) #prepare message to be sent to user
        msg.html = '<h1 style="text-align: center;">Hey ' + str(email) + "!</h1></br>" + "<h2 style='text-align: center;'>We have received your application to get your usernames.</h2></br>" + '<h3 style="text-align: center;">Here is a list of your usernames:</h3></br><h3 style="text-align: center;">' + "</br>".join(usernames) + "</h3>" + '</br><h4 style="text-align: center;">If you have any queries you may send an email to us at tourmanagementproj@gmail.com!</h4>' #code for body of email
        mail.send(msg) #send message to user
        ...
        if password != "": #if user keyed in a password
            if password == confirm: #if both password fields match
                ...
                if check is None: #username is not taken
                    ...
                    db.session.commit() #save changes
                    msg = Message('Tour Management Account Details for ' + str(username), sender = 'tourmanagementproj@gmail.com', recipients = [email]) #prepare message to be sent to user
                    msg.html = '<h1 style="text-align: center;">Hey ' + str(username) + "!</h1></br>" + "<h2 style='text-align: center;'>We have received your application for an account.</h2></br>" + '<h3 style="text-align: center;">Verify your account <a href=' + "https://tourmanagement.pythonanywhere.com/confirmation/" + getHashed(username) + ">here</a>!</h3>" + '</br><h4 style="text-align: center;">If you have any queries you may send an email to us at tourmanagementproj@gmail.com!</h4>' #code for body of email
                    mail.send(msg) #send message to user
...
def contact(): #contact function
    if request.method == "GET": #user loads the page
        ...
        msg = Message('Query from ' + str(username), sender = 'tourmanagementproj@gmail.com', recipients = ['tourmanagementproj@gmail.com']) #prepare email
...
#tour page which branches out into tour creation, tour management, and tour finding
@app.route("/tour", methods=["GET"]) #URL for tour page
def tour(): #tour function
    username = getUsername() #get username
    return render_template("tour.html", username=username, success=request.args.get("success"), info="You can create new tour or access and manage previous tour from here and discover tour. You need to be logged in to create and manage tour.")

#tour creation page where users can make new tour
@app.route("/tour/create_tour", methods=["GET", "POST"]) #URL for tour creation
def createTour(): #function to create tour
    if request.method == "GET": #user loads page
        username = getUsername() #get username
        if username != None: #user logged in
            return render_template("create_tour.html", username=username, info="Create a new tour here.") #render page with information
        else: #not logged in
            return render_template("create_tour.html", username=username, error="You aren't logged in. Any tour you try to create will not be saved.") #render page with error message
    elif request.method == "POST": #user submits form
        name = request.form['name'] #get name of tour
        username = getUsername() #get username
        description = request.form['description'] #get tour description
        tour = Tour(name=name,user=username,description=description) #prepare tour
        check = Tour.query.filter_by(name=name).first() #check if tour with similar name exists
        if check is None: #no tour with same name
            if username != None:
                db.session.add(tour) #add tour
                db.session.commit() #save changes
                return redirect(url_for("tour", success="Tour successfully created! It can be found under find tour.")) #return to tour page with message
            else: #user not logged in
                return render_template("create_tour.html", username=username, error="I told you that it would not work.") #render page with error message
        else: #name already used
            return render_template("create_tour.html", error="Invalid name.", username=username) #return and get user to use another name

#allows user to see all their tour
@app.route("/tour/manage_tours", methods=["GET", "POST"]) #URL to see all tour
def manageTours(): #function to see all tour
    if request.method == "GET": #user loads page
        username = getUsername() #get username
        ...
        if success is None: #not redirected
            tour = Tour.query.filter_by(user=username).all() #get all tour belonging to user
            if username is None: #user not signed in
                return render_template("manage_tours.html", error=error, success="Found all tour. However, you aren't logged in, so there isn't anything.", username=username, tour=tour, info="View all your tour here.") #return page and message
            elif tour is None: #no tour
                return render_template("manage_tours.html", error=error, success="You have not created any tour yet, so we can't show anything. Head over to the create tour page to create a new one!", info="View all your tour here.") #return page and message
            else: #signed in and has tour
                return render_template("manage_tours.html", error=error, success="Found all of your tour!", username=username, tour=tour, info="View all your tour here.") #return page and message
        else: #redirected
            tour = Tour.query.filter_by(user=username).all() #get all tour belonging to user
            return render_template("manage_tours.html", error=error, success=success, username=username, tour=tour, info="View all your tour here.") #return page and message

#tour searching which works even if user is not signed in
@app.route("/tour/find_tours", methods=["GET", "POST"]) #URL to search for tour
def findTours(): #function to find tour
    if request.method == "GET": #user loads page
        username = getUsername() #get username
        tour = Tour.query.all() #lists all tour
        success = request.args.get("success") #success message if redirected from another page
        error = request.args.get("error") #error message if redirected from another page
        if success != None:
            return render_template("find_tours.html", username=username, tour=tour, success=success, error=error, info="Find tour here.") #render page
        else:
            return render_template("find_tours.html", username=username, tour=tour, success="Found all tour!", error=error, info="Find tour here.") #render page
        ...
        if searchType == "user": #search for user
            tour = Tour.query.filter_by(user=search).all() #get tour
        elif searchType == "id": #search for ID
            tour = Tour.query.filter_by(id=search).all() #get tour
        elif searchType == "name": #search by name
            tour = Tour.query.filter_by(name=search).all() #get tour #Please fix the in statement so it works if "He" in "Hello"
        elif searchType == "description": #search by description
            tour = Tour.query.filter_by(description=search).all() #get tour #Please fix the in statement so it works if "He" in "Hello"
        return render_template("find_tours.html", username=username, tour=tour, info="Find tour here.", success="Search sucessful!") #return with results

#allows user to edit details for a specific tour
@app.route("/tour/edit/<id>", methods=["GET", "POST"]) #URL for tour editing
def editTour(id): #function to edit tour
    if request.method == "GET": #user loads page
        username = getUsername() #get username
        tour = Tour.query.filter_by(id=id).first() #get tour based on variable in URL
        if tour != None: #tour exists
            if username == tour.user: #if user is the creator of the tour
                return render_template("edit_tour.html", username=username, tour=tour, info="Edit details for this tour here.") #load page
            else: #user did not create this tour
                tour = Tour(id=id,name="HEY!",user="Somebody Else",description="This isn't your tour, so don't touch it.") #Troll tour
                return render_template("edit_tour.html", username=username, tour=tour, info="This form is useless since the tour doesn't belong to you.") #load page
        else: #no such tour
            tour = Tour(id=id,name="HEY!",user="To be discovered",description="This tour doesn't even exist yet. Or it has been deleted.") #Troll tour
            return render_template("edit_tour.html", username=username, tour=tour, info="This form is useless since the tour doesn't exist.") #load page
        ...
        password = request.form["password"] #to delete tour
        tour = Tour.query.filter_by(id=id).first() #get tour based on id passed in
        username = getUsername() #get username
        if tour != None: #tour exists
            if tour.user == username: #user is creator of tour
                if name != "": #user wants to change name
                    checkTour = Tour.query.filter_by(name=name).first() #check if name is used
                    if checkTour is None: #name not used
                        forms = Form.query.filter_by(tour=tour.name).all() #get all forms
                        for form in forms: #loop over forms
                            form.tour = name #change tour name linked to form
                        tour.name = name #change name
                if description != "": #user wants to change description
                    tour.description = description #change description
                if password != "": #user wants to delete tour
                    hashedPassword = getHashed(password) #get hashed version
                    ...
                    if user.password == hashedPassword: #correctpassword
                        db.session.delete(tour) #delete tour
                db.session.commit() #save changes
                return redirect(url_for('manageTours', success="Tour successfully edited.")) #return to manage tour with success message
            else: #not authorised
                return redirect(url_for('manageTours', error="You aren't authorised to edit this tour")) #return to manage tour with error message
        else: #no such tour
            return redirect(url_for('manageTours', error="No such tour.")) #return to manage tour with error message

#lets users see the form menu for an tour
@app.route("/tour/forms/<id>", methods=["GET", "POST"]) #URL forr forms menu for a tour
def forms(id): #function to open form menu for tour
    if request.method == "GET": #user loads page
        username = getUsername() #get username
        tour = Tour.query.filter_by(id=id).first() #find tour
        if tour != None: #tour exists
            if username == tour.user: #user authorised
                return render_template("forms.html", username=username, tour=tour, info="Create or manage forms for this tour here.", success="Found your tour!") #render page
            else: #user not authorised
                tour = Tour(id=id,name="HEY!",user="Somebody Else",description="This isn't your tour, so don't touch it.") #Troll tour
                return render_template("forms.html", username=username, tour=tour, info="Create or manage forms for this tour here, but nothing will happen since it isn't yours", success="Found a tour!") #render page
        else: #no such tour
            tour = Tour(id=id,name="HEY!",user="To be discovered",description="This tour doesn't even exist yet. Or it has been deleted.") #Troll tour
            return render_template("forms.html", username=username, tour=tour, info="Create or manage forms for this tour here, but we can't do anything since the tour doesn't exist.", success="Found a tour!") #render page

#allows users to create a new form
@app.route("/tour/forms/create_form/<id>", methods=["GET", "POST"]) #URL to create a new form
def createForm(id): #function to create a form
    if request.method == "GET": #user loads page
        ...
        tour = Tour.query.filter_by(id=id).first() #get tour
        if tour != None: #if tour exists
            if tour.user == username: #if user is authorised
                ...
                if check is None: #name not used
                    ...
                    form = Form(name=name,user=username,tour=tour.name,description=description,questions="[]",answers="/home/TourManagement/mysite/csv/" + str(lastForm.id+1) +".csv") #prepare form
                    db.session.add(form) #add form
                    db.session.commit() #save changes
                    return redirect(url_for("manageTours", success="Form successfully created!"))
                else: #name is used
            ...
            return render_template("create_form.html", username=username, error="You aren't authorised to create a form for this tour.", info="Create a new form here.") #return and tell user error message
        return render_template("create_form.html", username=username, error="Tour does't even exist.", info="Create a new form here.") #return and tell user error message

#allows users to see a forms and manage each one of them
@app.route("/tour/forms/manage_forms/<id>", methods=["GET", "POST"]) #URL to manage all forms
def manageForms(id): #function to see all forms
    if request.method == "GET": #user loads page
        ...
        forms = Form.query.filter_by(tour=tour.name).all() #get all forms
        if tour != None: #tour exists
            if username == tour.user: #user is authorised
                ...
                forms = [Form(id=id,name="HEY!",user="Somebody Else",tour="Unknown",description="This isn't your tour, so don't touch it.",questions=[],answers="")] #troll form
            ...
            forms = [Form(id=id,name="HEY!",user="To be discovered",tour="Unknown", description="The tour doesn't even exist yet. Or it has been deleted.",questions=[],answers="")] #troll form
            return render_template("manage_forms.html", username=username, forms=forms, success="Found a form!", info="Manage all your forms here.") #render troll page

#lets users edit form details
@app.route("/tour/forms/edit/<id>", methods=["GET", "POST"]) #URL to edit forms
    ...
    if request.method == "GET": #user loads page
        ...
        if form != None: #if form exists
            if username == form.user: #user authorised
                ...
                form = Form(id=id,name="HEY!",user="Somebody Else",tour="Unknown",description="This isn't your form, so don't touch it.",questions=[],answers="") #troll form
            ...
            form = Form(id=id,name="HEY!",user="To be discovered",tour="Unknown",description="The form doesn't even exist yet. Or it has been deleted.",questions=[],answers="") #troll form
        ...
        if form != None: #form exists
            if form.user == username: #user authorised
                ...
                db.session.commit() #save changes
                return redirect(url_for('manageTours', success="Form successfully edited.")) #return with success message
            else: #user not authorised
                return redirect(url_for('manageTours', error="You aren't authorised to edit this form.")) #return with error message
        else: #no such form
            return redirect(url_for('manageTours', error="The form doesn't exist.")) #return with error message

#function to allow people to edit form questions
@app.route("/tour/forms/questions/<id>", methods=["GET", "POST"]) #URL to edit form questions
    ...
    if request.method == "GET": #user loads page
        ...
        if form != None: #form exists
            if username == form.user: #user is authorised
                ...
                form = Form(id=id,name="HEY!",user="Somebody Else",tour="Unknown",description="This isn't your form, so don't touch it.",questions=[[1,"Are you dumb?","This is not a test.","radio",["YES","NO"]],[2, "Should you be here?","Is this your tour?","radio",["YES","NO"]]],answers="") #troll form
        ...
        if "addField" in request.form: #user wants to add a field
            ...
            if form != None: #form exists
                if form.user == username: #user is authorised
                    ...
                    return redirect(url_for("manageTours", success="Question successfully added."))
                else: #user not authorised
                    form = Form(id=id,name="HEY!",user="Somebody Else",tour="Unknown",description="This isn't your form, so don't touch it.",questions=[[1,"Are you dumb?","This is not a test.","radio",["YES","NO"]],[2, "Should you be here?","Is this your tour?","radio",["YES","NO"]]],answers="") #troll form
                    questions = form.questions #get questions
            ...
            if form != None: #form exists
                if form.user == username: #user authorised
                    ...
                    with open(form.answers, 'a') as csvFile: #open CSV file
                        ...
                        csvFile.close() #close CSV file
                    return redirect(url_for("manageTours", success="Question successfully edited.")) #return with success message
                else: #user not authorised
                    form = Form(id=id,name="HEY!",user="Somebody Else",tour="Unknown",description="This isn't your form, so don't touch it.",questions=[[1,"Are you dumb?","This is not a test.","radio",["YES","NO"]],[2, "Should you be here?","Is this your tour?","radio",["YES","NO"]]],answers="") #troll form
                    questions = form.questions #get questions
                    return render_template("forms_questions.html", username=username, form=form, questions=questions, error="You aren't allowed to edit this form.", info="You can edit and add questions to your form here.") #load troll page
...
@app.route("/tour/forms/answers/<id>", methods=["GET"]) #URL to view form answers
...
@app.route("/tour/forms/answers/download/<id>", methods=["GET"]) #URL to download answer data
...
#allows users to view details for an tour
@app.route("/tour/view/<id>", methods=["GET"]) #URL to view an tour
def viewTour(id): #function to view an tour
    if request.method == "GET": #user loads page
        username = getUsername() #get username
        tour = Tour.query.filter_by(id=id).first() #get tour
        if tour != None: #tour exists
            return render_template("view_tour.html", username=username, tour=tour, success="Found the tour.", info="View details for an tour here.") #render page with message
        else: #no such tour
            return render_template("view_tour.html", username=username, tour=tour, info="View details for an tour here. But there's nothing as the tour doesn't exist.") #render page with message

#allows user to view forms linked to an tour
@app.route("/tour/view/forms/<id>", methods=["GET"]) #URL to view all forms
def viewForms(id): #function to show all forms
    if request.method == "GET": #user accesses page
        username = getUsername() #get username
        tour = Tour.query.filter_by(id=id).first() #get tour
        forms = Form.query.filter_by(tour=tour.name).all() #get all forms linked to tour
        if forms != None: #has forms linked to tour
            return render_template("view_forms.html", username=username, forms=forms, success="Found all forms.", info="View all forms for an tour here.") #render page with message
        else: #if no forms linked to tour
            return render_template("view_forms.html", info="View all forms for an tour here. This tour does not contain any forms, so there isn't anything.") #render page with message

#allows user to fill in a form
@app.route("/tour/view/forms/fill_form/<id>", methods=["GET","POST"]) #URL to fill in a form
def fillForm(id): #function to fill in a form
    if request.method == "GET": #user loads page
        ...
        if form != None: #formm exists
            ...
            form = Form(id=id,name="HEY!",user="To be discovered",tour="Unknown", description="The tour doesn't even exist yet. Or it has been deleted.",questions=[],answers="")
        ...
        if form != None: #form exists
            ...
            if questions != []: #form has questions
                ...
                with open(form.answers, 'a') as csvFile: #open CSV data file
                    ...
                    csvWriter.writerow(row) #write out CSV row
                    csvFile.close() #close CSV file
                return redirect(url_for("findTours", success="Your response has been recorded.")) #return to find tour page
            else: #form doesn't have questions
                return redirect(url_for("findTours", error="Form doesn't have any questions.")) #return to find tour page with message
        else: #form doesn't exist
            return redirect(url_for("findTours", error="Form doesn't exist.")) #return to find tour page with message
    ...
    if request.method == "GET": #user loads page
        ...
        if username != None: #user is logged in
            if hashedPassword != user.password: #wrong password
                ...
                msg = Message('Tour Management Account Details for ' + str(username), sender = 'tourmanagementproj@gmail.com', recipients = [email]) #prepare message to be sent to user
                msg.html = '<h1 style="text-align: center;">Hey ' + str(username) + "!</h1></br>" + "<h2 style='text-align: center;'>We have received your request to delete your account.</h2></br>" + '<h3 style="text-align: center;">Re-verify your account <a href=' + "https://tourmanagement.pythonanywhere.com/confirmation/" + getHashed(username) + ">here</a>!</h3>" + '</br><h4 style="text-align: center;">If you have any queries you may send an email to us at tourmanagementproj@gmail.com!</h4>' #code for body of email
                mail.send(msg) #send message to user
                db.session.commit() #save changes
                session.pop('username', None) #remove user session
